...
object Ex0Wordcount {
  ...
  def wordcount(): RDD[(String, Int)] = {
    loadData().map(w => (w, 1)).reduceByKey(_ + _)
  }
/**
 * Attention, ici la transformation retourne un rdd, pas besoin d'affectation dans une variable
 * Pas non plus besoin d'une action car les tests gèrent cela
 * */
