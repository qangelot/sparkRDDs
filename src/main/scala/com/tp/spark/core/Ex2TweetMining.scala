...
object Ex2TweetMining {
   ...
   *  Attention ici, le caractère @ peut indiquer une mention, mais également être une abréviation isolée : "This morning I have been @ the gym! Feeling so nice :)"
   */
  def mentionOnTweet(): RDD[String] = {
    loadData().flatMap(_.text.split(" ").filter(_.startsWith("@")).filter(_.length > 1))
  }
  ...
  def countMentions(): RDD[(String, Int)] = {
    mentionOnTweet().map(x => (x, 1)).reduceByKey(_ + _)
  }
